...
# Kein Test, sondern eine Ausgabehilfe: Ohne diese Marke sammelt pytest sie
# wegen des Namens ein und meldet "fixture 'name' not found".
...
def test_apr_ampr_preflight():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Die Quality Suite prüft hier den Aufgaben-7-Preflight, nicht die
            # separat paketierte MkPFS-Implementierung. Eine lokale Attrappe
            # macht die erwartete Indexschnittstelle reproduzierbar verfügbar.
            fake_mkpfs = types.ModuleType("mkpfs")
            fake_mkpfs.__path__ = []
            fake_ampr = types.ModuleType("mkpfs.ampr")

            def ensure_ampr_index(root_path, *, enabled=True):
                index_path = Path(root_path) / "ampr_emu.index"
                index_path.write_bytes(
                    b"AMPRIDX3\\n"
                    b"/app0/fakelib/libSceAmpr.sprx\\n"
                    b"/app0/fakelib/libScePlayGo.sprx\\n"
                )
                return index_path if enabled else None

            fake_ampr.ensure_ampr_index = ensure_ampr_index
            fake_mkpfs.ampr = fake_ampr
            with patch.dict(
                sys.modules,
                {"mkpfs": fake_mkpfs, "mkpfs.ampr": fake_ampr},
            ):
            ...
            no_apr_root.mkdir()
            gui._ask_yesno_threadsafe = lambda *_args, **_kw: (_ for _ in ()).throw(
...
def test_universal_export_targets():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            checks = {
                # Seit der ".ffpfs (unkomprimiert)"-Zielformat-Erweiterung ist
                # "ffpfs" zusätzlich zu "ffpfsc" ein gültiges Ziel für jede
                # packbare Quelle.
                "Dump-Ziele": matrices["folder"] == ("ffpfsc", "ffpfs", "exfat", "ffpkg"),
                "FFPFSC-Ziele": matrices["ffpfsc"]
                == ("folder", "ffpfs", "exfat", "ffpkg"),
                "exFAT-Ziele": matrices["exfat"]
                == ("folder", "ffpfsc", "ffpfs", "ffpkg"),
                "FFPKG-Eingabeziele": matrices["ffpkg"]
                == ("folder", "ffpfsc", "ffpfs", "exfat"),
                "FFPKG-Ziel für packbare Quellen": all(
                    "ffpkg" in matrices[source_type]
                    for source_type in ("folder", "ffpfsc", "exfat")
                ),
                "FFPKG nicht als Selbstziel": "ffpkg" not in matrices["ffpkg"],
                "Ausgewählter FFPKG-Wert bleibt erhalten": gui.target_format.get()
                == ".ffpkg",
                "Combobox mit FFPKG": gui.format_combo["values"]
                == [".ffpfsc", ".ffpfs (unkomprimiert)", ".exFAT", ".ffpkg"],
...
def test_ffpfsc_verification_and_task7_repack():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Zielformat "Dump-Ordner": Eine falsch verschachtelte Quelle liefert
            # einen Ordner mit einem einzigen Container statt eines Game-Dumps.
            # Die Abschlussprüfung muss das erkennen, sonst gilt der Lauf als
            # erfolgreich und der Fehler faellt erst auf der PS5 auf.
            gui._current_language = "de"
            gui.target_format = types.SimpleNamespace(get=lambda: "Dump-Ordner")

            no_dump = Path(temp_dir) / "kein_dump"
            no_dump.mkdir()
            (no_dump / "inner_image.exfat").write_bytes(b"x" * 4096)
            verify_no_dump = gui._verify_output_artifact("universal_convert", str(no_dump))

            real_dump = Path(temp_dir) / "echter_dump"
            (real_dump / "sce_sys").mkdir(parents=True)
            (real_dump / "eboot.bin").write_bytes(b"y" * 4096)
            verify_real_dump = gui._verify_output_artifact("universal_convert", str(real_dump))

            batch_root = Path(temp_dir) / "sammelziel"
            (batch_root / "spiel_a").mkdir(parents=True)
            (batch_root / "spiel_a" / "eboot.bin").write_bytes(b"z" * 4096)
            verify_batch = gui._verify_output_artifact("batch_convert", str(batch_root))
        ...
        def _funktionsquelle(name: str) -> str:
            """Schneidet den Quelltext einer Methode bis zur naechsten heraus."""
            start = source.index(f"    def {name}(")
            naechste = source.index(chr(10) + "    def ", start + 1)
            return source[start:naechste]

        ampr_source = _funktionsquelle("_mode_ampr_manager")
        unpack_exfat_source = _funktionsquelle("_mode_unpack_to_exfat")
        inner_source = _funktionsquelle("_extract_inner_image")
        ...
        checks = {
            ...
            # Ohne --raw legt mkpfs von sich aus noch ein Image dazwischen; das
            # Ergebnis ist dann dreifach verschachtelt und auf der PS5 unbrauchbar.
            # Identische Anforderung wie in _mode_pack_folder_mkpfs (Aufgabe 1).
            "Inneres PFS wird roh gepackt (--raw)": '"--raw"' in helper_source,
            "Außencontainer folgt der Zielendung": (
                '"pack", "file"' in helper_source
                and '"--no-compress" if uncompressed else "--compress"' in helper_source
                and '.ffpfs' in helper_source
            ),
            "Ordner ohne Game-Dump wird abgelehnt": not verify_no_dump["ok"],
            "Echter Game-Dump wird akzeptiert": verify_real_dump["ok"],
            "Sammelziel mit Dump-Unterordnern wird akzeptiert": verify_batch["ok"],
            # Aufgabe 2, 4 und 7 packen jetzt ueber dieselbe Schleife aus
            # (_entpacke_container_ebenen). Frueher hatte jede ihre eigene
            # Fassung - und Korrekturen erreichten nur eine davon.
            "Aufgabe 7 nutzt die gemeinsame Auspack-Schleife": (
                "_entpacke_container_ebenen(" in ampr_source
            ),
            "Aufgabe 2 nutzt die gemeinsame Auspack-Schleife": (
                "_entpacke_container_ebenen(" in unpack_exfat_source
            ),
            "Gemeinsamer Weg erkennt UFS2-Innenimage": (
                'kind == "ufs2"' in inner_source
                and "_extract_ffpkg_to_folder_via_ufs2tool" in inner_source
            ),
            # Reihenfolge zaehlt: erst der eigene exFAT-Leser, OSFMount nur,
            # wenn der scheitert.
            "Gemeinsamer Weg hat nativen exFAT-Fallback vor OSFMount": (
                0
                < inner_source.index("_extract_exfat_to_folder_mkpfs")
                < inner_source.index("_extract_exfat_via_osfmount")
            ),
        }
    ...
    try:
        ...
        checks = {
            ...
            "Aufgabe 6 unterstützt Ordner-Vorschau": (
                '"pack_folder", "ampr_manager", "universal_convert"' in source
            ),
            ...
            "Hersteller wird in Infobox angezeigt": (
                '("publisher", self._t("info_popup.meta.publisher"))' in source
            ),
            "Format und Metadatenmethode werden angezeigt": (
                'self._t("info_popup.format_label")' in source
                and 'self._t("info_popup.metadata_label")' in source
            ),
        }
