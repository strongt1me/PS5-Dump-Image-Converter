...
namespace UFS2Tool
{
    ...
    public class Ufs2Image : IDisposable
    {
        ...
        // --- Streamende Lesevariante (kein 2-GB-byte[]-Limit) ----------------
        // Spiegelt ReadFile()/ReadIndirectBlock() usw., haelt aber nie die ganze
        // Datei im Speicher: jeder Datenblock geht direkt in den Ausgabe-Stream.
        // Sparse-Loecher werden ueber CatchUpZeros mit Nullen gefuellt, damit die
        // Ausgabe byteweise ReadFile() entspricht. Behebt die int32-Grenze, an der
        // Extraktion von Dateien >2 GB scheiterte.
        public void ReadFileToStream(uint inodeNumber, Stream output)
        {
            var inode = ReadInode(inodeNumber);
            if (!inode.IsRegularFile && !inode.IsSymlink)
                throw new InvalidOperationException($"Inode {inodeNumber} is not a regular file or symlink.");
            ...
            long offset = 0;
            long written = 0;
            byte[] blockBuf = new byte[Superblock.BSize];
            byte[] zeroBuf = new byte[Superblock.BSize];

            for (int i = 0; i < Ufs2Constants.NDirect && offset < size; i++)
            {
                long toAdvance = Math.Min((long)Superblock.BSize, size - offset);
                if (inode.DirectBlocks[i] != 0)
                {
                    CatchUpZeros(output, offset, ref written, zeroBuf);
                    _stream.Position = inode.DirectBlocks[i] * Superblock.FSize;
                    ReadFully(blockBuf, 0, (int)toAdvance);
                    output.Write(blockBuf, 0, (int)toAdvance);
                    written += toAdvance;
                }
                offset += toAdvance;
            }

            if (offset < size && inode.IndirectBlocks[0] != 0)
                offset = StreamIndirectBlock(inode.IndirectBlocks[0], output, offset, size, ref written, blockBuf, zeroBuf);
            if (offset < size && inode.IndirectBlocks[1] != 0)
                offset = StreamDoubleIndirectBlock(inode.IndirectBlocks[1], output, offset, size, ref written, blockBuf, zeroBuf);
            if (offset < size && inode.IndirectBlocks[2] != 0)
                offset = StreamTripleIndirectBlock(inode.IndirectBlocks[2], output, offset, size, ref written, blockBuf, zeroBuf);

            // Restliche Sparse-Region bis zur Dateigroesse mit Nullen auffuellen.
            CatchUpZeros(output, size, ref written, zeroBuf);
        }

        private void CatchUpZeros(Stream output, long targetOffset, ref long written, byte[] zeroBuf)
        {
            long gap = targetOffset - written;
            while (gap > 0)
            {
                int chunk = (int)Math.Min((long)zeroBuf.Length, gap);
                output.Write(zeroBuf, 0, chunk);
                written += chunk;
                gap -= chunk;
            }
        }

        private long StreamIndirectBlock(long indirectBlockFrag, Stream output, long offset, long fileSize, ref long written, byte[] blockBuf, byte[] zeroBuf)
        {
            long blockOffset = indirectBlockFrag * Superblock.FSize;
            _stream.Position = blockOffset;
            ...
            for (int i = 0; i < pointersPerBlock && offset < fileSize; i++)
            {
                ...
                int toRead = (int)Math.Min(Superblock.BSize, fileSize - offset);
                ReadFully(blockBuf, 0, toRead);
                output.Write(blockBuf, 0, toRead);
                written += toRead;
                offset += toRead;
        ...
        private long StreamDoubleIndirectBlock(long doubleIndirectBlockFrag, Stream output, long offset, long fileSize, ref long written, byte[] blockBuf, byte[] zeroBuf)
        {
            long blockOffset = doubleIndirectBlockFrag * Superblock.FSize;
            _stream.Position = blockOffset;
            ...
            for (int i = 0; i < pointersPerBlock && offset < fileSize; i++)
            {
                ...
                offset = StreamIndirectBlock(ptr, output, offset, fileSize, ref written, blockBuf, zeroBuf);
                _stream.Position = blockOffset + ((long)(i + 1) * ptrSize);
            }
        ...
        private long StreamTripleIndirectBlock(long tripleIndirectBlockFrag, Stream output, long offset, long fileSize, ref long written, byte[] blockBuf, byte[] zeroBuf)
        {
            long blockOffset = tripleIndirectBlockFrag * Superblock.FSize;
            _stream.Position = blockOffset;
            ...
            for (int i = 0; i < pointersPerBlock && offset < fileSize; i++)
            {
                ...
                offset = StreamDoubleIndirectBlock(ptr, output, offset, fileSize, ref written, blockBuf, zeroBuf);
                _stream.Position = blockOffset + ((long)(i + 1) * ptrSize);
            }
        ...
        public void ExtractFile(uint inodeNumber, string outputPath)
        {
            ...
            // Streamen statt ReadFile()+WriteAllBytes: kein 2-GB-byte[]-Limit,
            // damit Dateien > 2 GB korrekt extrahiert werden (int32-Grenze behoben).
            using var fs = new FileStream(outputPath, FileMode.Create, FileAccess.Write,
                                          FileShare.None, 1 << 20);
            ReadFileToStream(inodeNumber, fs);
        }
