...
![Version](https://img.shields.io/badge/Version-v1.8.90-blue)
...
> **Neu bei der Bedienung?** Das **[Benutzerhandbuch](BENUTZERHANDBUCH.html)** (Version v1.8.90) erklärt Aufgabenwahl, Quelle und Ziel, Fortschrittsanzeige, alle Werkzeugleisten-Buttons sowie typische Fehler in einfacher Sprache – mit Inhaltsverzeichnis und allen acht Aufgaben. Es liegt zweifach bei: als **[BENUTZERHANDBUCH.html](BENUTZERHANDBUCH.html)** zum Lesen im Browser und als **[BENUTZERHANDBUCH.pdf](BENUTZERHANDBUCH.pdf)** (30 Seiten) zum Ausdrucken und Weitergeben.
...
**v1.8.90** lässt die **Quelle** beim Start leer – sie muss bewusst gewählt werden, damit kein unbedachter Klick den Dump der letzten Sitzung umwandelt. Das **Ziel** zeigt weiterhin den zuletzt benutzten Ordner, und der Durchsuchen-Knopf öffnet nach wie vor im letzten Quellordner. Details siehe [Release Notes v1.8.90](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.90).
...
**v1.8.76** blendet während einer PS4-Konvertierung viermal den Hinweis ein, dass das fertige Spiel **nur vom externen USB-Datenträger** starten darf und nie von der internen SSD – sonst gibt es beim Start einen Kernel Panic. Die Einblendung bleibt 15 Sekunden, blendet sich langsam ein und aus und braucht keinen Klick. Außerdem eine Richtigstellung: In v1.8.74 stand „Unterordner werden nicht durchsucht" – tatsächlich funktioniert `/mnt/usb0/homebrew/`, nur selbst angelegte Ordner werden nicht gefunden. Details siehe [Release Notes v1.8.76](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.76).

**v1.8.75** ändert nichts am Programm. Die Ausgabe bringt den vollständigen Testbestand mit – bisher lagen nur sieben von 58 Dateien im Repository – und behebt drei Ursachen dafür, dass Testläufe je nach Reihenfolge unterschiedlich ausgingen: eine beim Start angelegte und sofort wieder zerstörte Fensterwurzel, ein fest erwarteter deutscher Text und ein zurückgezogenes Fenster. Der Gesamtlauf liefert jetzt reproduzierbar 1155 grüne Prüfungen. Details siehe [Release Notes v1.8.75](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.75).
...
**v1.8.72** bringt `.ffpkg` auf **macOS und Linux**. Bisher hieß es im Programm, das gehe nur unter Windows – das war aber unsere Packentscheidung und keine Grenze des Werkzeugs: Das verwendete UFS2Tool läuft auf allen drei Systemen, es lag bei uns nur der Windows-Bau bei. Einzig das Einhängen als Laufwerk bleibt Windows (Dokan-Treiber); anderswo wird direkt entpackt. Nebenbei fällt eine stille Voraussetzung weg: Der alte Bau verlangte ein installiertes **.NET 8** und scheiterte ohne es kommentarlos. Die neuen Bauten bringen alles mit. Details siehe [Release Notes v1.8.72](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.72).

**v1.8.71** sorgt dafür, dass bei kurzen Fenstern nichts mehr aus dem Bild fällt. Der Inhalt rechts braucht rund 1356 Pixel Höhe; war das Fenster kürzer als etwa 880, stand der Rest **außerhalb des Fensters** – bei 768 Pixeln fehlten STARTEN und ABBRECHEN, nicht verkleinert, sondern unerreichbar. Der Inhalt lässt sich jetzt rollen; reicht der Platz, ändert sich nichts. Außerdem nennt der **Diagnosebericht** jetzt Fassung und Herkunft aller mitgelieferten Werkzeuge, und ein Knopf **„Aktualisierungen prüfen“** fragt auf Wunsch die Quellen ab und zeigt, wo es etwas Neueres gibt. Details siehe [Release Notes v1.8.71](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.71).

**v1.8.70** räumt auf, was die neue Darstellungsprüfung an schmalen Fenstern gemeldet hat. Die **Werkzeugleiste** oben brauchte für ihre dreizehn Knöpfe rund 1515 Pixel; war weniger da, wurden sie zusammengedrückt statt weggelassen – bei 1366 Pixeln Fensterbreite war „BENUTZERHANDBUCH" noch **26 Pixel** breit. Jetzt wandern einzelne Knöpfe ins Sammelmenü und kommen beim Verbreitern zurück. Die **Statuszeile** bricht um, statt abgeschnitten zu werden. Die Mindestbreite des Fensters steigt auf 1230 Pixel. Außerdem wird **kein Hintergrundbild mehr hochgerechnet**: Das zu kleine `ray-burst` ist ersetzt, die Seitenleistenbilder liegen in 640 statt 320 Pixeln Breite bei. Nachgemessen bei 1230 bis 1920 Pixeln: keine einzige Auffälligkeit mehr. Details siehe [Release Notes v1.8.70](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.70).

**v1.8.69** bringt das Hintergrundbild dahin, wo es hingehört. Beim Öffnen des Fensters wurde es **gar nicht auf die Fenstergröße gerechnet** – es blieb in der Größe der Bilddatei stehen, während Inhaltsfläche und Seitenleiste längst angepasst waren; sichtbar als Bruch zwischen den Flächen, der erst verschwand, wenn man das Fenster einmal anfasste. Ein **Designwechsel** hinterließ dieselbe Art Fehler: Die Oberfläche meldet dabei kurz eine Zwischengröße, und die dafür berechnete Fassung überschrieb kurz darauf die richtige. Beides ist behoben. Die **Einstellungen** nennen jetzt außerdem die Bildmaße, die dein Bildschirm wirklich braucht, statt einer festen Zahl – bei 125 % Anzeigeskalierung ist die Seitenleiste rund 500 statt der bisher angegebenen 320 Pixel breit. Gefunden hat die neue Prüfung dabei gleich noch etwas: Die Zeile mit **AMPR EMU und BACKPORT** brauchte 1145 Pixel Breite und stand bei jedem Fenster unter rund 1725 Pixeln teilweise außerhalb der Pfad-Karte – nicht zu sehen und nicht anzuklicken. Die beiden haben jetzt eine eigene Zeile; die Mindestbreite des Fensters steigt dafür von 1100 auf 1200 Pixel. Und der **Diagnosebericht** prüft die Darstellung mit: zusammengedrückte und abgeschnittene Bedienelemente, zu enge Beschriftungen, hochgerechnete Bilder, Anzeigeskalierung, Arbeitsspeicher und Reaktionszeit des Fensters. Details siehe [Release Notes v1.8.69](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.69).

**v1.8.60** behebt, dass man in **elf von vierzehn** Werkzeugfenstern nicht an die unterste Knopfreihe kam, ohne das Fenster erst größer zu ziehen. Die Maße standen als feste Zahlen im Quelltext; wächst der Inhalt, bleibt die Zahl stehen und „Schließen“ rutscht hinaus. Am schlimmsten traf es den AMPR-Index-Builder, dem **265 Pixel Höhe** fehlten. Jedes Fenster fragt jetzt seinen eigenen Inhalt, was er braucht, und wächst darauf – der Bildschirm ist die Obergrenze. Gefunden wurde das durch Ausmessen aller Fenster, nachdem eine Bildschirmaufnahme den Verdacht geliefert hatte. Außerdem lässt die **Protokollfläche** jetzt 20 % des Hintergrundbildes durchscheinen. Details siehe [Release Notes v1.8.60](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.60).

**v1.8.59** räumt die Erstinstallation auf dem Mac auf. `Install_macOS.sh` entfernt die Quarantäne-Markierung längst – lag aber **nicht im `.dmg`**, sondern nur im Repository. Wer die App von Hand herauszog, behielt die Markierung und wurde von macOS blockiert. Das Abbild enthält jetzt zusätzlich **„Erste Installation.command“** zum Doppelklicken und eine Verknüpfung auf `/Applications`. Außerdem erkennt das Programm, wenn macOS es aus einem **Schattenordner** startet (App Translocation) – dort gehen Einstellungen und Protokolle beim Beenden verloren – und sagt, wie man es behebt. Die **Schrift auf dem Mac** ist jetzt so groß wie unter Windows: 1,65 statt 1,35, am Diagnosebericht nachgerechnet. Details siehe [Release Notes v1.8.59](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.59).

**v1.8.58** macht Fehler sichtbar, die es bisher gar nicht bis zum Nutzer schafften. Das Programm wird als Fensteranwendung gebaut – damit ist `sys.stderr` leer, und genau dorthin schreibt Tkinter jeden Fehler aus einem Knopf oder einer Tastenbindung. **Ein Fehler in der Oberfläche verschwand also spurlos:** kein Protokolleintrag, keine Meldung, der Knopf tat einfach nichts. Drei Haken fangen sie jetzt ab – Tk-Ereignisse, Hauptfaden und die Arbeitsfäden – und schreiben sie mit vollständiger Rückverfolgung ins Protokoll. Der **Diagnosebericht** wuchs dazu um sechs Abschnitte: Anzeige (Bildschirm, DPI, Skalierung, Schriften), Laufzeitumgebung, gefundene Fremdwerkzeuge, freier Speicherplatz, die Fehler dieser Sitzung und die Protokolldatei. Details siehe [Release Notes v1.8.58](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.58).

**v1.8.57** sagt vor der Wahl, was ein PFS-Container kostet: Die PS5 entpackt `.ffpfsc` und `.ffpfs` mit rund **150–250 MB/s** – etwa ein Drittel eines USB-Laufwerks und ein Zehntel der internen SSD. Spiele, die viel nachladen, können dadurch ruckeln. Der Hinweis steht jetzt unter der Zielformat-Liste und erscheint auch dann, wenn man nur das Format umstellt statt die Aufgabe. Ebenfalls in dieser Version: die **Entscheidung gegen** eine kleinere Blockgröße – gemessen bringt sie an der fertigen Datei 0,1 %. Details siehe [Release Notes v1.8.57](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.57).

**v1.8.56** gibt die Prüfung nach dem Packen frei. mkpfs prüft von sich aus die Struktur eines fertigen Abbilds — das Programm hat diese Prüfung bisher an sechs Stellen **abgeschaltet**, ohne dass man es sehen oder ändern konnte. Die offizielle Anleitung und das Referenzprogramm lassen sie laufen. Neben den Worker-Threads steht jetzt ein Feld **PRÜFUNG** mit drei Stufen: *Aus* (wie bisher), *Schnell* (Voreinstellung, entspricht mkpfs) und *Vollständig* (liest das Ergebnis zusätzlich komplett zurück). Details siehe [Release Notes v1.8.56](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.56).

**v1.8.55** räumt vier Befunde von echter Apple-Hardware weg. Der schwerste: Ein Klick auf FileZilla ließ die App **vollständig abstürzen** – ein Dateiname als Muster im Dateidialog, den Tk auf macOS nicht verarbeiten kann. Der Absturzbericht führte zu einer zweiten Stelle mit demselben Fehler im SELF-Inspektor. Außerdem wird FileZilla auf macOS und Linux jetzt überhaupt gefunden, das Hintergrundbild passt sich im Vollbild an und wird **formatfüllend beschnitten statt verzerrt**, und alle Schriften sind auf macOS größer – dort bedeutet dieselbe Punktzahl rund ein Drittel weniger Pixel als unter Windows. Details siehe [Release Notes v1.8.55](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.55).

**v1.8.54** berichtigt zwei Fehler aus v1.8.53. Der sichtbare: Aufgabe 8 meldete FEHLGESCHLAGEN, obwohl sie die Ursache im selben Lauf behoben hatte — die `param.json` wurde erst *nach* der Prüfung angelegt, das Urteil stand da schon fest. Sie wird jetzt vorher behandelt. Der stillere: Beim Zusammenlegen der beiden Rückfragen ging verloren, dass die Frage auf **Nein** vorbelegt war, wenn ein Ja die Titel-ID an prosperopatches.com schickt; ein versehentliches Enter löste den Netzabruf mit aus. Außerdem sprechen die beiden Prüfaufgaben nicht mehr von einer „Konvertierung“, die sie gar nicht durchführen. Details siehe [Release Notes v1.8.54](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.54).

**v1.8.53** holt die Angaben für eine fehlende `param.json` aus dem Backup selbst. Bisher kannte das Programm nur zwei Quellen: `sce_sys/nptitle.dat` für die Titel-ID und einen Online-Nachschlag für Titel und Content-ID. Dazu kommen jetzt drei lokale: die `eboot.bin` (Titel-ID, falls `nptitle.dat` fehlt), der Trophäen-Container `trophy2/trophy00.ucp` (der **Anzeigename** steht dort als lesbares JSON) und `sce_sys/pfs-version.dat` (die **Inhaltsversion**). Letztere ist der wichtigste Zugewinn: Bisher wurde pauschal `01.000.000` eingetragen — bei einem gepatchten Spiel schlicht falsch, ohne dass man es der Datei ansieht. Außerdem genügt jetzt **eine** Bestätigung statt drei, und die selbst erzeugte `param.json` besteht die eigene Prüfung — was seit v1.8.51 nicht der Fall war. Details siehe [Release Notes v1.8.53](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.53).

**v1.8.52** räumt zwei Darstellungsfehler weg, die bei einer Durchsicht der Oberfläche aufgefallen sind. Im **hellen Design** blieb das dunkle Hintergrundbild mit voller Deckkraft stehen, während Karten und Knöpfe hell wurden – das Fenster zerfiel sichtbar in zwei Hälften. Beim **Programmstart** stand rund eine Sekunde lang das Standardsymbol von Tkinter in der Taskleiste, weil das eigene Symbol erst nach dem Aufbau der Oberfläche gesetzt wurde. Dazu sitzen die **Spaltenüberschriften** in allen Tabellen jetzt bündig über ihren Werten statt zentriert – bei einem breit gezogenen Fenster standen sie weit auseinander. Details siehe [Release Notes v1.8.52](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.52).

**v1.8.51** prüft die `sce_sys/param.json` jetzt inhaltlich – und repariert sie auf Wunsch. Bisher genügte es, dass die Datei sich als JSON lesen ließ; alles Weitere fiel erst auf der Konsole auf. Eine Versionsnummer als Zahl statt als Zeichenkette, eine `contentId`, die eine andere Title-ID nennt als das Feld daneben, ein fehlender Sprachblock oder ein BOM am Dateianfang führen dort zu „Missing/invalid param.json" – der Dump sieht dabei einwandfrei aus. Diese Fälle erkennt das Programm jetzt beim Bau **und im Validator**, und es bietet an, sie zu berichtigen. Repariert heißt dabei wirklich repariert: Vorhandene Angaben bleiben stehen, die alte Fassung wird als `param.json.alt` danebengelegt. Details siehe [Release Notes v1.8.51](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.51).

**v1.8.50** bringt die Anwendung auf den Mac. Neben der Windows-EXE und der Linux-Programmdatei entsteht mit `./Build_macOS.sh` ein Programmbündel `PS5 Dump & Image Converter.app` – mit Symbol im Dock, eigenem Namen in der Menüleiste, scharfer Darstellung auf Retina-Bildschirmen und dunklem Erscheinungsbild. `./Install_macOS.sh` legt es in den Programme-Ordner. Es gelten dieselben Einschränkungen wie unter Linux: `.ffpkg` lesen und bauen sowie die OSFMount-Ersatzwege bleiben Windows vorbehalten, alles Übrige steht vollständig zur Verfügung. Details siehe [Release Notes v1.8.50](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.50).

**v1.8.49** räumt den Bibliothekssatz auf. Beim Backport wurde bisher der komplette mitgelieferte Ordner in das Spiel kopiert – im FW7-Satz also auch eine `ps5-backpork.elf`, der 116 KB große Payload des Referenzwerkzeugs. ShadowMount+ hängt den Ordner nach `common/lib`, wo Bibliotheken nach Namen geladen werden; nach dieser Datei fragt kein Spiel. Übernommen werden jetzt nur `.sprx` und `.prx` sowie die leere Markierung `FW<n>`, und was übersprungen wurde, steht im Protokoll. Dass die Datei verzichtbar ist, zeigt der Satz selbst: In FW4, FW5 und FW6 fehlt sie. Details siehe [Release Notes v1.8.49](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.49).

**v1.8.44** räumt das Protokollfeld wirklich auf. Text und Fortschrittsbalken klebten dort in einer Zeile (`Writing PFS image to …[####] 72% write`), und weil eine solche Zeile nicht als Balken erkannt wurde, stapelten sich die folgenden Balken statt sich zu ersetzen. Ursache war der Zeilentrenner der Engine-Ausgabe: Er suchte erst nach dem Zeilenumbruch und erst danach nach dem Wagenrücklauf. Jetzt wird am zuerst auftretenden getrennt – je Phase steht eine Zeile, die sich fortschreibt. Details siehe [Release Notes v1.8.44](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.44).

**v1.8.43** fasst die Verbindungsdaten der PS5 an einer Stelle zusammen: In den **EINSTELLUNGEN** stehen jetzt IP-Adresse, FTP-Port und KLOG-Port, dazu ein Knopf zum Testen. Alle Fenster, die eine Verbindung brauchen, schlagen diese Werte vor; stimmt ein Port nicht, probieren sie die bekannten Alternativen selbst durch. Der Knopf **KLOG** prüft vor dem Öffnen, ob klogsrv überhaupt läuft, und bietet sonst an, den Payload zu senden – über den Loader oder per FTP auf einen USB-Datenträger der Konsole, samt Eintrag in `autoload.txt`. Dazu übersichtlichere **BIBLIOTHEK** mit Rollbalken und sortierbaren Spalten, ein aufgeräumtes Protokollfeld und eine Fortschrittsanzeige ohne langen Stillstand. Details siehe [Release Notes v1.8.43](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.43).

**v1.8.42** bringt den Knopf **BENUTZERHANDBUCH** in die Titelleiste, links neben EN: Ein Druck öffnet das mitgelieferte Handbuch im Browser. Im Fenster EINSTELLUNGEN stand der Hinweis über den Knöpfen halb unter ihnen und war abgeschnitten – er hat jetzt eine eigene Zeile. Außerdem erscheinen Fehlermeldungen wieder, die bisher stumm blieben: Schlug ein Backport fehl oder ließ sich eine Remote-INI nicht laden, schreiben oder ihr Debug-Log nicht holen, blieb die Statuszeile einfach stehen, statt den Grund zu nennen. Details siehe [Release Notes v1.8.42](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.42).

**v1.8.41** verdoppelt die Auswahl der Hintergrundbilder für den Hauptbereich von zehn auf zwanzig. Die zehn neuen Bilder tragen dieselben Motive wie die hohen Bilder der Seitenleiste – Polarlicht, Lichtstrahlen, Bokeh, Sternenfeld, Höhenlinien, Wellenringe, Fluchtpunktraster, Punktraster und warme Bänder –, sodass sich beide Bereiche zueinander passend einstellen lassen. Wie die bisherigen sind sie bewusst dunkel gehalten, damit Karten und Beschriftungen davor lesbar bleiben. Details siehe [Release Notes v1.8.41](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.41).

**v1.8.40** räumt beim FILEZILLA-Knopf auf: Der eingebaute FTP-Client entfällt, der Knopf startet ausschliesslich Ihre eigene FileZilla-Installation. Gefunden wird sie jetzt auch dann, wenn der Installationsordner beliebig heißt oder direkt auf einem Laufwerk liegt – und der gefundene Pfad wird gemerkt, sodass der nächste Start ohne Suche auskommt. Dasselbe gilt für OSFMount. Details siehe [Release Notes v1.8.40](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.40).

**v1.8.39** machte den Programmstart ruhig. Das Fenster blitzte kurz **weiß** auf und baute sich danach sichtbar auf; in der Seitenleiste stand der Spielname zeitweise **über** dem Cover, das Bild verschwand für einen Moment und wanderte anschließend mehrfach. Beides ist behoben: Das Fenster erscheint erst fertig, Cover und Name sitzen von Anfang an fest. Dazu ein etwas größerer Spielname, runde Ecken am Startbildschirm und ein nachgezogener Dokan-2-Dialog. Details siehe [Release Notes v1.8.39](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.39).

**v1.8.38** behob einen Fehler, der **jeden Upload auf die Konsole** betraf: Seit dem 15.08. starteten dort hochgeladene Spiele nicht mehr (**CE-107750-0**). Ursache war der auf Tempo optimierte Payload **zftpd**, der Dateien ohne Ausführungsrecht ablegt – die PS5 startet solche Dateien nicht. Das Programm nutzt jetzt wieder **ftpsrv auf Port 2121** und prüft nach jedem Upload die Rechte, damit so etwas nicht noch einmal stumm bleibt. Details siehe [Release Notes v1.8.38](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.38).

**v1.8.37** behob beschnittene Bedienelemente in fünf Fenstern. Im **BACKPORT**-Fenster blieben die drei Knöpfe unbeschriftet und die Firmware-Auswahl leer, beim **PKG-Merger** fehlte die Knopfleiste ganz; dieselbe Ursache traf **DOWNLOADS**, **ShadowMount+/MicroMount** und den **JS Loader**. Ein neuer Test misst die Fenster jetzt aus. Siehe [Release Notes v1.8.37](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.37).

**v1.8.36** schloss die letzte Lücke der `param.json`-Reparatur: Fehlt oder zerbricht die Datei, konnte die Ersatzdatei bisher nur die Title-ID tragen – Titel und Content-ID stehen in **keiner** lokalen Datei eines Dumps. Beide lassen sich jetzt auf Wunsch zur Title-ID online nachschlagen. Die Frage danach ist eine eigene, auf **Nein** voreingestellte Rückfrage, die nennt, wohin die Title-ID geht; ohne Netz oder bei Ablehnung entsteht die Ersatzdatei wie bisher. Nebenbei behoben: Das Fenster *Spiel-Info* zeigte den Titel mit vorangestellter Title-ID (`PPSA19015: Arcade Game Zone`). Siehe [Release Notes v1.8.36](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.36).

**v1.8.35** brachte **BACKPORT** in das Menü **WEITERE TOOLS**: Ein Spiel, das ein zu neues SDK verlangt, lässt sich damit auf eine ältere Firmware (4.00 bis 7.00) herabsetzen. Das Programm entpackt jede ausführbare Datei, setzt die SDK-Angabe im Modulkopf herab, signiert sie neu und legt die passenden Ersatzbibliotheken dazu. Auf Wunsch entsteht vorher eine vollständige Sicherung; ersetzt wird eine Datei erst, wenn Patchen *und* Signieren gelungen sind. Alles läuft ohne Fremdwerkzeug und ohne .NET-Laufzeit. Siehe [Release Notes v1.8.35](https://github.com/strongt1me/PS5-Dump-Image-Converter/releases/tag/v1.8.35).
...
| Windows-EXE-Ziel | `dist\PS5_Dump_Image_Converter_v1.8.90.exe` |
...
Die Aufgabenbezeichnungen entsprechen der aktuellen Oberfläche von v1.8.90.
...
2. `PS5_Dump_Image_Converter_v1.8.90.exe` starten.
...
```text
dist\PS5_Dump_Image_Converter_v1.8.90.exe
```
...
```text
dist/PS5_Dump_Image_Converter_v1.8.90_linux_x86_64
```
...
Aufgabe 1 ("Dump-Ordner konvertieren") und Aufgabe 3 ("exFAT konvertieren") bieten im Zielformat-Dropdown jetzt zusätzlich **`.ffpfs` (unkomprimiert)** neben `.ffpfsc`. Strukturell identisch zum inneren PFS-Image – lediglich der äußere Kompressions-Schritt (`--compress`/`--no-compress` bei MkPFS) wird übersprungen. Ergebnis: größere Datei, aber schnellerer Schreib-/Lesevorgang ohne Dekompression. Auch in Aufgabe 5 (Sammelkonvertierung) und Aufgabe 6 (AIO) als Zielformat wählbar. Als reine Eingabe wurde `.ffpfs` bereits zuvor vom Validator/der Bibliothek erkannt – diese Änderung ergänzt die fehlende Ausgabeseite.
...
Die PyInstaller-Spezifikation erzeugt den synchronisierten v1.8.90-Zielnamen. PyInstaller bündelt die Python-Anwendung für Windows.[6]
...
| `BENUTZERHANDBUCH.html` | Bedienanleitung zum Lesen im Browser (v1.8.90, mit Inhaltsverzeichnis, alle Aufgaben und Werkzeugleisten-Buttons) |
