...
class WidgetbaumTests(unittest.TestCase):
    ...
    def test_in_der_ecke_steht_der_hintergrund(self) -> None:
        ...
        Gegenprobe zum stillen Ausfall: Wäre die Rundung nicht gelaufen,
        stünde in **allen vier** Ecken beidesmal dasselbe.

        Geprüft wird deshalb die **deutlichste** der vier Ecken, nicht jede
        einzelne. Der Grund steckt im Verfahren: Außen steht ein örtlicher
        Ausschnitt des Hintergrundbilds, innen die Kartenfläche. Trifft eine
        Ecke zufällig auf eine Bildstelle, die der Kartenfarbe gleicht, sind
        beide Punkte dort fast gleich – ohne dass etwas kaputt wäre. Am
        26.08.2026 gemessen: Die Protokollkarte kam links oben auf 6, in den
        übrigen drei Ecken deutlich darüber.

        Der stille Ausfall bliebe trotzdem hängen – er beträfe alle vier.
        """
        ...
        for name in ("path_card", "console_frame"):
            ...
            with self.subTest(karte=name):
                self.assertTrue(bilder, "%s ohne Eckbilder" % name)
                abstaende = {}
                for ecke, schild in sorted(bilder.items()):
                    foto = getattr(schild, "_bild", None)
                    self.assertIsNotNone(foto, "%s/%s ohne Bild" % (name, ecke))
                    r = foto.width()
                    (ax, ay), (ix, iy) = punkte[ecke]
                    aussen = self._punkt(foto, ax % r, ay % r)
                    innen = self._punkt(foto, ix % r, iy % r)
                    abstaende[ecke] = sum(abs(a - b)
                                          for a, b in zip(aussen[:3], innen[:3]))
                self.assertGreaterEqual(
                    max(abstaende.values()), MINDESTABSTAND,
                    "%s: keine einzige Ecke hebt sich ab (%s) - "
                    "die Rundung ist offenbar gar nicht gelaufen"
                    % (name, abstaende))
