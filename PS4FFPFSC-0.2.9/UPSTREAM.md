...
Grundlage ist **PS4 FFPFSC 0.2.9** (`PS4pkg_to_ffpfsc`, 01.09.2026), lizenziert
unter GPL-3.0-or-later. Der vollständige Lizenztext liegt als `LICENSE` daneben,
die Lizenzen der weiterverwendeten Fremdbestandteile in `LICENSES/` und
`THIRD_PARTY_NOTICES.md`.

## Stand 17.09.2026: von 0.2.8 auf 0.2.9 gehoben

Der Autor hat in 0.2.9 genau fünf Dateien angefasst; drei davon liegen hier:
`__init__.py` (Versionszeile), `dlc_embed.py` (zwei Meldungstexte) und
`pipeline.py`. `gui.py` und `gui_model.py` gehören zur Qt-Oberfläche, die hier
bewusst fehlt. Alle übrigen Dateien sind zwischen 0.2.8 und 0.2.9 unverändert –
byteweise geprüft.

In `pipeline.py` steckt die eigentliche Neuerung: `extractor_partial_destination`
legt den Teilbestand des Entpackers unter Windows in einen ASCII-Zwischenordner
(`_extractor_staging/<Title-ID>/…`), wenn der übliche Pfad Sonderzeichen
enthielte, und `_remove_empty_extractor_staging` räumt ihn wieder weg. Das
betrifft dieselbe Stelle, an der dieses Projekt seit dem 23.08.2026 den zu
langen Pfad von „Paket verschlüsselt“ unterscheidet.

Zusammengeführt wurde dreiwegig (Original 0.2.8 als Grundlage, unser Stand und
Original 0.2.9). Es gab **einen** Konflikt, beim Aufräumen nach einem
gescheiterten Entpacklauf. Aufgelöst so: Das Löschen des Teilbestands geht jetzt
gegen `settings.unpacked_dir` statt gegen `root` – der Teilbestand kann durch den
neuen Zwischenordner außerhalb des Spielordners liegen –, danach räumt
`_remove_empty_extractor_staging` auf, und erst dann kommen unsere beiden
Meldungen (zu langer Pfad, Absturz des Entpackers) wie bisher.

Die Programme in `bin/` (`ps4_pkg_extract`, `ps4-dlc-patch`, je Windows und
macOS) stammen ebenfalls aus 0.2.9 – aus den Veröffentlichungen
`PS4-FFPFSC-v0.2.9-windows-x64.zip` und `…-macos-arm64.zip`, deren SHA-256 der
Autor selbst veröffentlicht; die Windows-Prüfsumme wurde vor dem Übernehmen
verglichen. Gegenüber 0.2.8 sind sie **gleich groß** und unterscheiden sich nur
in der Prüfsumme: derselbe Quellstand, neu gebaut. Der Ordner heißt seit dem
17.09.2026 `PS4FFPFSC-0.2.9`.
