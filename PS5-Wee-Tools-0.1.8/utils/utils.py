...
class Config:
	...
	def __init__(self, file='config.ini'):
		self.file = file
		self.path = file if os.path.isabs(file) else os.path.join(ROOT_PATH, file)
...
class Beep:
	...
	@classmethod
	def success(cls):
		if not cls.IS_ACTIVE or not winsound: return
...
class UI:
	...
	@classmethod
	def findActionEx(cls, actions, str = ''):
		for action in actions:
			r = cls.findAction(action, str)
			if r: return r
		return False
	...
	def getMenu(menu, start=0):
		...
		if type(menu) is dict:
			...
			for n, text in enumerate(menu):
				fmt = '%d'
				if len(menu) >= 10: fmt = '%2d'
				if len(menu) >= 100: fmt = '%3d'
				lines.append(' '+fmt%(n+start)+': '+text)
	...
	@classmethod
	def showStatus(cls, force=False):
		if force:
			cls.STATUS_TXT = force
		if cls.STATUS_TXT:
